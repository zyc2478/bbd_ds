...
public class QuartzJobConfig {
    ...
    public MethodInvokingJobDetailFactoryBean bidListJobBean(BidListJob bidListJob) {
        ...
        jobDetail.setName("general-bidListJob"); // 任务的名字
        jobDetail.setGroup("general"); // 任务的分组
        jobDetail.setTargetObject(bidListJob); // 被执行的对象
        jobDetail.setTargetMethod("executeJob"); // 被执行的方法
        return jobDetail;
    }
    ...
    public CronTriggerFactoryBean bidListJobTrigger(@Qualifier("bidListJobBean") MethodInvokingJobDetailFactoryBean bidListJobBean) throws IOException {
        ...
        tigger.setCronExpression(cronStr); // 什么是否触发，Spring Scheduler Cron表达式
